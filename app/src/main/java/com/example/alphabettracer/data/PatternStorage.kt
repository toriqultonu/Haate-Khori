...
/**
 * Storage for Pattern Recognition Game progress
 */
object PatternStorage {
    private const val PREFS_NAME = "pattern_game_prefs"
    private const val KEY_HIGH_SCORE = "high_score"
    private const val KEY_TOTAL_CORRECT = "total_correct"
    private const val KEY_GAMES_PLAYED = "games_played"
    private const val KEY_BEST_STREAK = "best_streak"
    private const val KEY_PATTERNS_SOLVED = "patterns_solved"
    ...
    fun getPatternsSolved(context: Context): Int =
        getPrefs(context).getInt(KEY_PATTERNS_SOLVED, 0)

    fun incrementPatternsSolved(context: Context) {
        val current = getPatternsSolved(context)
        getPrefs(context).edit().putInt(KEY_PATTERNS_SOLVED, current + 1).apply()
    }
}
